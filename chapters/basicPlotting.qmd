...
### Plot Legends
Often when your figure contains more than one plot, it is helpful to include a plot legend to label the plots. To add a legend, you must first use the keyword argument `label` to each `plot` command to label the plots.  To add the legend, use the command `pyplot.ledend()`.  An example is given below:
...
## Exercises
1. If you were to shine a laser through a very narrow slit, you would see a pattern very similar to the one shown below. The function that describes this light pattern is called a sinc function:$$ y(x) = \left({\sin x\over x}\right)^2$$ Plot this function from $-20 < x < 20$.  Then make the following customizations to the plot: [![Single Slit Diffraction]](https://commons.wikimedia.org/wiki/File:Diffraction_of_a_single_slit.jpg#/media/File:Diffraction_of_a_single_slit.jpg)
