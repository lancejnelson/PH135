...
4. The quadratic equation has two solutions and they can be positive, negative, or complex.  Often the complex and negative solutions are not relavant/meaningful.  Write an `if/elif/else` block of code that checks what type of solutions will results and only displays the real, positive solution(s) if there is one.  If there is no positive solution, the code should display a message indicating such.  Then check your code with the values given below. As a reminder, here is the quadratic equation: $$ x = {- b \pm \sqrt{b^2 - 4 a c} \over 2a}  $$
...
5. (Triangle Classification)  There is a simple criteria to determine whether a triangle can be formed from a set of specified side lengths.  The criteria states that the sum of any two sides must be larger than the third side.  This criteria must be satisified for all possible combinations of two sides if the triangle is to be valid.  If the triangle is valid, the angles can be found by first using the law of cosines to find the first two angles and then choosing the third angle so that the sum of the three angles add up to $180^\circ$. $$c^2 = a^2 + b^2 - 2 a b \cos(C)$$

    1. Write some code that checks to see if a valid triangle can be formed from the given side lengths.  Check your code for several choices of side lengths to verify that it is working.
    2. If a triangle can be formed, use the law of cosines to calculate the triangle's angles.
