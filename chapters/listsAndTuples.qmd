...
4. (Fibonacci Series)  The Fibonacci series is a very famous sequence of numbers that appears repeatedly in nature. In the cell below you will find a list that contains the first seven Fibonacci numbers.
    ...
    3. Another way to generate the Fibonacci sequence is to use the following formula: $F(n) = {1\over \sqrt{5}}\left({1 + \sqrt{5}\over 2} \right)^n - {1\over \sqrt{5}}\left({1 - \sqrt{5}\over 2} \right)^n$ ($n$ starts at zero). Use this formula together with the append (or extend) function to generate a list containing the first 15 Fibonacci numbers.
    4. The Fibonacci sequence has some very interesting properties (listed below).  Add some code in the cell below to verify each of them.
...
6. (Lucas Numbers) The Lucas numbers are another sequence of numbers closely related to the Fibonacci numbers.  The first two numbers of the sequence are $2$,$1$.  The rest of the numbers in the sequence can be generated in several ways.  Construct this sequence of numbers using the following methods:
    ...
    2. The Lucas numbers can be generated by adding up alternating Fibonacci numbers: $L_n = F_{n-1} + F_{n + 1}$. Use the `append` method and this formula to build a list containing the first 15 Lucas numbers. (Note: You'll need a list containing at least $16$ Fibonacci numbers to do this.)
    ...
    4. The Lucas sequence has some very interesting properties (listed below).  Add some code in the cell below to verify each of them.
        1. The ratio of the last two numbers in the sequence gets closer and closer to the 'Golden Ratio' (1.618...) as the sequence gets longer.  Calculate this ratio for short sequences and then increase the length of the sequence and watch the ratio approach 1.618...
        2. The sum of any ten consecutive Lucas numbers is divisible by 11.  Check this for at least three different length-ten slices.
        3. The sum of the first $n$ Lucas numbers is equal to the $(n + 2)$-th Lucas number minus 1.  Check this for at least three different values of $n$.
```{python}
lucas = [2,1]
```